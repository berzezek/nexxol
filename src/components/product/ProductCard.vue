...
<div class="col mb-5">
  <div class="card h-100 shadow">
      ...
      <div class="card-footer p-4 pt-0 border-top-0 bg-transparent">
          <div class="text-center"><a class="btn btn-outline-dark mt-auto" href="#">Подробнее</a></div>
      </div>
  </div>
</div>
</template>
...
export default {
  ...
  computed: {
    ...
    // }),
    beautyPrice() {
      return Math.floor(this.product.price).toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ");
    },
    beautyDiscount() {
      return Math.floor(this.product.discount_price).toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ");
    },
  },
  mounted() {
